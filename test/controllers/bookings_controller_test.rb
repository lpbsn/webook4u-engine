...
class BookingsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "GET #new renders the page for a valid slot without creating a booking" do
    travel_to Time.zone.local(2026, 3, 15, 8, 0, 0) do
      slot = Time.zone.local(2026, 3, 16, 10, 0, 0)

      assert_no_difference "Booking.count" do
        get new_service_booking_path(@client.slug, @service, start_time: slot, enseigne_id: @enseigne.id)
      end

      assert_response :success
      assert_includes response.body, @enseigne.name
      assert_includes response.body, @enseigne.full_address
      assert_includes response.body, "Continuer"
    end
  end

  test "GET #new redirects to client page with alert when start_time is invalid" do
    travel_to Time.zone.local(2026, 3, 15, 8, 0, 0) do
      get new_service_booking_path(@client.slug, @service, start_time: "invalid", enseigne_id: @enseigne.id)
  ...
  test "GET #new keeps date context on redirect when start_time is invalid" do
    travel_to Time.zone.local(2026, 3, 15, 8, 0, 0) do
      get new_service_booking_path(@client.slug, @service, start_time: "invalid", enseigne_id: @enseigne.id, date: "2026-03-16")
  ...
  test "GET #new redirects to client page with alert when slot is not generated by the system" do
    travel_to Time.zone.local(2026, 3, 15, 8, 0, 0) do
      ...
      assert_no_difference "Booking.count" do
        get new_service_booking_path(@client.slug, @service, start_time: slot_not_in_grid, enseigne_id: @enseigne.id)
      end
  ...
  test "GET #new redirects to client page with alert when start_time is beyond max_future_days" do
    travel_to Time.zone.local(2026, 3, 15, 8, 0, 0) do
      ...
      get new_service_booking_path(@client.slug, @service, start_time: time_beyond, enseigne_id: @enseigne.id)
  ...
  test "GET #new returns 404 for unknown client slug" do
    travel_to Time.zone.local(2026, 3, 15, 8, 0, 0) do
      slot = Time.zone.local(2026, 3, 16, 10, 0, 0)
      get new_service_booking_path("slug-inconnu-xyz", @service, start_time: slot, enseigne_id: @enseigne.id)
  ...
  test "GET #new returns 404 for inactive enseigne" do
    inactive_enseigne = @client.enseignes.create!(name: "Inactive", full_address: "2 rue de Paris", active: false)

    travel_to Time.zone.local(2026, 3, 15, 8, 0, 0) do
      slot = Time.zone.local(2026, 3, 16, 10, 0, 0)
      get new_service_booking_path(@client.slug, @service, start_time: slot, enseigne_id: inactive_enseigne.id)
      assert_response :not_found
    end
  end
